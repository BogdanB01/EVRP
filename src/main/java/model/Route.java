...
public class Route {
    ...
    public int evalRoute() {
        int score = 0;
        int numberOfCustomers = getNumberOfCustomers();
        if (!areTimeWindowsViolated()) {
            score += 50 * numberOfCustomers;
        }
        if (!isTankCapacityViolated()) {
            score += 40 * numberOfCustomers;
        }
        if (!isPayloadCapacityViolated()) {
            score += 30 * numberOfCustomers;
        }
        return score;
    }

    public void insertRechargingStation(RechargingStation rechargingStation) {
      addNode(rechargingStation);
    }
    ...
    private void appendRoute(Route route) {
        ...
        nodes = Stream.of(nodes, routeToAppend).flatMap(List::stream).collect(Collectors.toList());
    }
